...
```
:::
Here we are given *a lot* of information. We can see the function name, the details, how it's called, and so forth. In the **Arguments** section we get a list of all of the arguments we can pass to the function, along with a description of them. In this case we see that `round` can take a parameter called `x` for the number to be rounded, and `digits` (which we have previously seen).
