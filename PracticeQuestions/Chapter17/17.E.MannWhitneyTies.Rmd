...
unique_vals <- sample(ceiling(N/2):(N-1), 1)
...
combined_data <- round(unname(sort(rnorm(unique_vals, mu, sd))), rd)
if(any(combined_data == 0)) {
    combined_data[which(combined_data==0)] <- 1/(10^rd)
}

if(length(unique(combined_data)) < unique_vals) {
    # There are duplicates
    for(idx in 1:(unique_vals-1)) {
        idxs <- idx + which(combined_data[(idx+1):unique_vals] == combined_data[idx])
        combined_data[idxs] <- combined_data[idx] + 1/(10^rd)
    }
}

combined_data <- combined_data[c(1:unique_vals, sample(1:unique_vals, N-unique_vals, TRUE))]
...
for(x in X) {
    Rank_X <- Rank_X + rank(combined_data)[which(combined_data == x)[1]]
}

Rank_Y <- 0

for(y in Y) {
    Rank_Y <- Rank_Y + rank(combined_data)[which(combined_data == y)[1]]
}
...
ties <- as.numeric(table(combined_data))
sigma_Usq <- n*m*(n+m+1)/12 - n*m*sum(ties^3 - ties)/(12*(n+m)*(n+m-1))

p_val <- 2 * pnorm(-abs(U - n*m/2)/sqrt(sigma_Usq))
